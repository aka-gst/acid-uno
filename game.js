...
function startGame() {
  ...
  /*
    Уровень висит на корне: по нему CSS решает, насколько
    громко подсвечивать подходящие карты. Ставится здесь,
    а не в лобби, потому что первая партия раздаётся при
    загрузке страницы.
  */
  ...
  */
  ...
  /*
    На спокойном столе первую строку пишет v9.1.js: там она
    зависит от того, есть ли чем ходить, и «перетащи карту»
    было бы неправдой, когда подходящей карты нет.
  */
  if (!calmMode) {
...
function fanPosition(
  ...
  /*
    Дуга веера. Крайние карты уходили ниже низа контейнера
    (curve давал +7), а с наклоном их угол вылезал уже за
    нижнюю кромку экрана — на телефоне две крайние карты
    оказывались срезаны. Теперь на краях дуга не опускается
    ниже нуля.
  */
  const y =
    phoneLandscape
