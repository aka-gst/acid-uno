...
(() => {
    ...
    document
      ...
      .forEach(
        el => {
          ...
          /*
            Карта, которой можно сходить, поднимается и в
            стопке, а не только вверх.

            Замер на телефоне 390×844, рука из семи: пальцу
            доступно 33–63% площади карты, и у одной центр
            принадлежал соседке — нажатие в середину карты
            брало другую. Веер уложен по порядку, z-index
            1..7, каждая следующая накрывает предыдущую;
            играбельная поднималась на 44 точки вверх, но
            оставалась под соседкой справа. Торчит из веера,
            а нажать нельзя.

            Порядок между самими играбельными сохраняем:
            прибавляем сотню к их собственному номеру, а не
            равняем всех.

            Правится здесь, а не в стилях: z-index ставится
            инлайном при отрисовке руки, и любой инлайн
            сильнее правила в css.
          */
          const base =
    ...
    */
    ...
    others.forEach((seat, index) => {
      ...
      */
      ...
      /*
        Как в референсе: у сидящих по бокам портрет стоит с
        внешней стороны, ближе к краю экрана, а карты уходят
        внутрь, к столу. У сидящего сверху портрет над
        картами — сбоку ему там мешать некому.

        Без этого портреты всех троих жались влево, и стол
        выглядел сдвинутым.
      */
      /*
        Портрет сбоку почти всегда: так значок остаётся
        низким и целиком помещается над столом. Когда он
        стоял над картами, значок верхнего соперника вырастал
        вдвое и провисал внутрь стола.

        Исключение — тесный стол на пять-шесть соперников:
        там значкам не хватает ширины, и портрет уходит
        наверх.
      */
      const side =
        others.length <= 4 ||
  ...
  */
  function renderDirection91() {
    ...
    */
    ...
    /*
      Кольцо заодно и говорит, чей ход: кислотное, когда
      ходишь ты, и цвет стола, когда думает соперник.
      Направление и так видно по вращению.
    */
    el.classList.toggle(
      "is-mine",
  ...
  */
  function trail91(from, to, color, delay) {
    ...
    /*
      След рисуется от одной карты к другой. Если одна из них
      ещё не разложена — например, рука спрятана на время
      раздачи, — её прямоугольник нулевой и лежит в начале
      координат, а след превращается в полосу через весь
      экран. Это и была та самая полоска посреди экрана.
    */
    if (
      !from?.width ||
      !to?.width ||
      len < 40 ||
      len > Math.hypot(window.innerWidth, window.innerHeight) * .75
    ) {
      return;
    }
  ...
  */
  ...
  /* =======================================================
     ...
     Партия начиналась сразу собранной: все карты уже на
     руках, и было непонятно, что вообще произошло. Теперь
     карты вылетают из колоды по очереди — своя рука и вееры
     соперников, — а в конце объявляется, чей ход.

     Раздача чисто внешняя: состояние партии к этому моменту
     уже сложилось в редьюсере, здесь только показ. Ход
     соперника подождёт, пока она идёт.
     ======================================================= */
  ...
  */
  ...
  async function dealAnimation91() {
    ...
    */
    ...
    game.classList.remove("is-dealing");
