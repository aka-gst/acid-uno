...
/*
  ...
  gentle — спокойный стол: бот выбирает по мягкому раскладу
  и не копит штрафной кластер. Паузу за столом держит не
  он, а v9.1.js: за живым столом в комнате простого
  уровня нет.
*/
function decide(state, seat, roll, gentle) {
  ...
  /*
    Спокойный стол: штраф бот кроет не своим штрафом, а
    просто забирает. Кластер «+2 накрыли +4, стало +6» —
    первое, на чём новичок теряет нить: карты уезжают
    туда-сюда, и непонятно, кому в итоге платить.

    Заодно это и есть настоящая поблажка. Отказ копить
    кластер стоит боту партий: по прогонам новичок с ним
    выигрывает почти половину вместо трети, а штрафных
    карт берёт на четверть меньше.
  */
  if (
    gentle &&
    state.drawPenalty > 0
  ...
  if (moves.length > 0) {
    ...
    const chosen =
      hand[
        R.chooseCard(hand, indexes, roll, gentle)
      ];
