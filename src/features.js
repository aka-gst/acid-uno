...
(() => {
  ...
  */
  ...
  /*
    Кто ещё не выбирал — попадает в обучение.

    Раньше умолчанием была «ИГРА», и новый человек проходил
    мимо обучения, даже не узнав, что оно есть. Кто играл в
    УНО сто раз, переключится одним касанием и увидит, что
    выбор был; кто не играл — не окажется один за столом.

    Сохранённый выбор сильнее умолчания: выбрал «ИГРУ» —
    больше не спросим.
  */
  let chosenCalm =
    (() => {
      ...
      try {
        ...
        /* свой выбор сильнее любого умолчания */
        if (saved) {
          return saved === "calm";
        }

        /*
          Не выбирал — смотрим, проходил ли обучение.

          Не прошёл — попадает в обучение: иначе новый человек
          садится за стол без единой подсказки, и всё, что мы
          делали, до него не доезжает.

          Прошёл — сразу игра. Гнать в обучение второй раз
          значит превратить его в помеху; кому надо ещё раз,
          выберет режим руками.

          Метку ставит сам наставник, когда экскурсия
          заканчивается или её пропускают (src/coach.js,
          closeTour).
        */
        return localStorage.getItem("acid-uno-tour") !== "1";
